...
graph TB
    subgraph DataLayer["📦 Shared Data Layer - script.js"]
        IMG["COURSE_IMAGES<br/>base64 SVG thumbnails"]
        CATS["CATEGORIES<br/>filter list"]
        COURSESARR["COURSES array<br/>id, title, category, price,<br/>rating, image, tags"]
    end

    subgraph ExtraLayer["📦 Extended Data - course.js"]
        EXTRA["COURSE_EXTRA<br/>by course id:<br/>curriculum, outcomes,<br/>requirements, reviews, FAQ"]
    end
    ...
    subgraph Detail["📖 course.html - Detail Page"]
        HEADER["Course Header<br/>title, price, rating, enrol CTA"]
        CURRICULUM["Curriculum Accordion"]
        OUTCOMES["What You'll Learn"]
        REQ["Requirements"]
        REVIEWS["Reviews and FAQ"]
    end
    ...
    COURSESARR -->|lookup by id| HEADER
...
flowchart LR
    ...
    B -->|"Filter by category"| C["script.js filters COURSES array<br/>and re-renders grid"]
    C --> B
    B -->|"Click a course card"| D["Navigate to course.html?course=ID"]
    D --> E["course.js reads URL param 'course'"]
    E --> F["Find course in COURSES array<br/>and merge COURSE_EXTRA by ID"]
